...
using CaseFile.Api.County.Controllers;

namespace CaseFile.Api.County.Handlers
{
    public class CountiesCommandHandler : //IRequestHandler<GetCountiesForExport, Result<List<CountyCsvModel>>>,
        ...
        IRequestHandler<GetCities, Result<List<CityModel>>>
...

    {
        ...
        //    var countiesDictionary = counties.ToDictionary(c => c.UserId, y => y);
        ...
        //        {
        //            if (countiesDictionary.TryGetValue(county.CountyId, out var csvModel))
        //            {
        ...
        //                countiesIdUpdated.Add(county.CountyId);
        //            }
        //        }
        ...
        //{
        //    if (counties.Count != counties.Select(x => x.UserId).Distinct().Count())
        //    {
        ...
        public async Task<Result<List<CountyModel>>> Handle(GetAllCounties request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                counties = await _context.Counties
                    ...
                    .OrderBy(c => c.Name)
                    .Select(c => new CountyModel { Code = c.Code, CountyId = c.CountyId, Name = c.Name })
                    //.OrderBy(c => c.Order)
                    .ToListAsync(cancellationToken);
            }
            catch (Exception e)
            {
                ...
                return Result.Failure<List<CountyModel>>(e.Message);
            }
        ...
        public async Task<Result<List<CityModel>>> Handle(GetCities request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                cities = await _context.Cities
                    ...
                    .OrderBy(c => c.Name)
                    .Select(c => new CityModel { CityId = c.CityId, Name = c.Name })                    
                    .ToListAsync(cancellationToken);
            }
            catch (Exception e)
            {
        ...
        //    try
        //    {
        //        var county = await _context.Counties.FirstOrDefaultAsync(x => x.CountyId == request.CountyId, cancellationToken);
        //        if (county == null)
        //        {
        ...
        //    try
        //    {
        //        var county = await _context.Counties.FirstOrDefaultAsync(x => x.CountyId == request.County.CountyId, cancellationToken);
        //        if (county == null)
        //        {
        //            return Result.Failure($"Could not find county with id = {request.County.CountyId}");
        //        }
        ...
        //    catch (Exception e)
        //    {
        //        _logger.LogError($"Unable to update county {request.County.CountyId}", e);
        //        return Result.Failure($"Unable to update county {request.County.CountyId}");
        //    }
        //}
    }
}